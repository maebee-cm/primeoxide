...
impl BitVec {
    ...
    pub fn set_bit(&mut self, index: usize, value: bool) {
        ...
        if u64_index == self.data.len()-1 && bit_shift > self.bit_length {
            panic!("Index out of bounds, accessing uninitialized bits. Bit length is {} but the bit \
            index accessed is {}", self.bit_length, bit_shift);
        }
    ...
    pub fn get_population_count(&self, index: Option<usize>) -> usize {
        let index = if let Some(x) = index { x }
        else { (self.data.len()-1)*64+self.bit_length as usize };
        ...
        if last_u64_index > self.data.len() {
            panic!("Index out of bounds, accessing bytes beyond allocated memory. The len is {} but \
            the index is {}", self.data.len(), last_u64_index);
        }
        else if last_u64_index == self.data.len() && last_u64_bits > self.bit_length {
            panic!("Index out of bounds, accessing uninitialized bits. Bit length is {} but the bit \
            index accessed is {}", self.bit_length, last_u64_bits);
        }
